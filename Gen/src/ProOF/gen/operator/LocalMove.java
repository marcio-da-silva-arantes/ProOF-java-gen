...
public final class LocalMove extends Approach{
    ...
    public List<oLocalMove> list_moves(){
        return Arrays.asList(move);
    }
    public oLocalMove select(List<oLocalMove> movs) throws Exception {
        return movs.get(prob.rnd.nextInt(movs.size()));
    }
    public Solution local_search(Solution ind, oLocalMove mov) throws Exception {
        Solution sol = ind.clone(prob);
        mov.local_search(prob, sol.codif());
        return sol;
    }
}
